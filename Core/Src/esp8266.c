...
//设备单链接
//设备连接成功返回true， 反之false
bool ESP8266_Send_data_toServer (char * pStr)
{
     char cStr [20];
	 
    sprintf ( cStr, "AT+CIPSEND=%d", strlen(pStr));
...
bool ESP8266_TCP_Closed( void )
{
    if (!ESP8266_Send_AT_Cmd ( "AT+CIPCLOSE", "OK", 0, 500 ))
        return false;
