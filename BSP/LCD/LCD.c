 ...
 ****************************************************************************************************
 ...
 此版本移植与正点原子，该版本只适用于实验室3.0板子。
 实验室3.2的版本LCD驱动芯片为9488，将9488的驱动和配置代码加入该版本
 该版本目前通用于实验室的3.0和3.2的板子
 
 太湖学院物联网工程 张喆 2024-3-10
...
uint32_t lcd_read_point(uint16_t x, uint16_t y)
{
	...
	lcd_set_cursor(x,y);	    
	if(lcddev.id==0X9341||lcddev.id==0X9486||lcddev.id==0X9488||lcddev.id==0X6804||lcddev.id==0X5310||lcddev.id==0X1963)lcd_wr_regno(0X2E);//9341/6804/3510/1963 发送读GRAM指令
	else if(lcddev.id==0X5510)lcd_wr_regno(0X2E00);	//5510 发送读GRAM指令
	else lcd_wr_regno(0X22);      		 			//其他IC发送读GRAM指令
	if(lcddev.id==0X9320)lcd_opt_delay(2);				//FOR 9320,延时2us	    
 	r=lcd_rd_data();								//dummy Read	   
	if(lcddev.id==0X1963)return r;					//1963直接读就可以 
	lcd_opt_delay(2);	  
 	r=lcd_rd_data();  		  						//实际坐标颜色
 	if(lcddev.id==0X9341||lcddev.id==0X9486||lcddev.id==0X9488||lcddev.id==0X5310||lcddev.id==0X5510)		//9341/NT35310/NT35510要分2次读出
 	{
		lcd_opt_delay(2);	  
		b=lcd_rd_data(); 
...
//    }
	...
	if(lcddev.id==0X9341||lcddev.id==0X9486||lcddev.id==0X9488||lcddev.id==0X6804||lcddev.id==0X5310||lcddev.id==0X1963)lcd_wr_regno(0X29);	//开启显示
	else if(lcddev.id==0X5510)lcd_wr_regno(0X2900);	//开启显示
...
//    }
	if(lcddev.id==0X9341||lcddev.id==0X9486||lcddev.id==0X9488||lcddev.id==0X6804||lcddev.id==0X5310||lcddev.id==0X1963)lcd_wr_regno(0X28);	//关闭显示
	else if(lcddev.id==0X5510)lcd_wr_regno(0X2800);	//关闭显示
...
void lcd_set_cursor(uint16_t x, uint16_t y)
{
    ...
    else if (lcddev.id == 0x5510)
    {
		...
		else if(lcddev.id==0X9486||lcddev.id==0X9488)				//新增ili9486
		{		    
			lcd_wr_regno(0x002A);
			lcd_wr_data(x>>8);lcd_wr_data(0x00FF&x);                  
			lcd_wr_data(0x0001);lcd_wr_data(0x00df);

			lcd_wr_regno(0x002B);
			lcd_wr_data(y>>8);lcd_wr_data(0x00FF&y);                  
			lcd_wr_data(0x0001);lcd_wr_data(0x00df); 		
		}
...
void lcd_scan_dir(uint8_t dir)
{
    ...
    if ((lcddev.dir == 1 && lcddev.id != 0x1963) || (lcddev.dir == 0 && lcddev.id == 0x1963))
    {
		...
		if(lcddev.id==0x9341||lcddev.id==0X9486||lcddev.id==0X9488||lcddev.id==0X6804||lcddev.id==0X5310||lcddev.id==0X5510||lcddev.id==0X1963)//9341/6804/5310/5510/1963,特殊处理
		{
			switch(dir)
			{
...
void lcd_display_dir(uint8_t dir)
{
	if(dir==0)			//竖屏
	{
		...
		lcddev.width=240;
		lcddev.height=320;
		if(lcddev.id==0X9341||lcddev.id==0X9486||lcddev.id==0X9488||lcddev.id==0X6804||lcddev.id==0X5310)
		{
			lcddev.wramcmd=0X2C;
	 		lcddev.setxcmd=0X2A;
			lcddev.setycmd=0X2B;  	 
			if(lcddev.id==0X6804||lcddev.id==0X9486||lcddev.id==0X9488||lcddev.id==0X5310)
			{
				lcddev.width=320;
				lcddev.height=480;
			}
	...
	if(lcddev.id==0X9488) lcd_scan_dir(R2L_U2D);
	else lcd_scan_dir(DFT_SCAN_DIR);	//默认扫描方向
...
void lcd_set_window(uint16_t sx, uint16_t sy, uint16_t width, uint16_t height)
{
	...
	twidth=sx+width-1;
	theight=sy+height-1;
	if(lcddev.id==0X9341||lcddev.id==0X9486||lcddev.id==0X9488||lcddev.id==0X5310||lcddev.id==0X6804||(lcddev.dir==1&&lcddev.id==0X1963))
	{
		lcd_wr_regno(lcddev.setxcmd); 
		lcd_wr_data(sx>>8); 
		lcd_wr_data(sx&0XFF);	 
...
//    HAL_SRAM_Init(&g_sram_handle, &fsmc_read_handle, &fsmc_write_handle);
    ...
    if (lcddev.id != 0x9341 && lcddev.id !=0x9486 && lcddev.id !=0x9488)    /* 不是 9341 , 尝试看看是不是 ST7789 */
    {
        lcd_wr_regno(0x04);
        lcddev.id = lcd_rd_data();      /* dummy read */
    ...
    {
        lcd_ex_ili9488_reginit();       /* 执行ILI9486初始化 */
    }
...
//    }
    ...
    lcd_display_dir(1); /* 默认为竖屏 :0表示竖屏，1表示横屏*/
    LCD_BL(1);          /* 点亮背光 */
    lcd_clear(WHITE);
}
